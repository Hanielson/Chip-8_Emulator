...
int Chip8::draw(unsigned char reg_x_coord , unsigned char reg_y_coord , unsigned char rows){
    unsigned char xpos = 0x0;
    unsigned char ypos = 0x0;
    reg[0xF] = 0;
    ...
    for(unsigned char i = 0x0 ; i < rows ; ++i){
        if(((reg[reg_y_coord] + i)) >= 32){
            ypos = 32;
        } else{
            ypos = reg[reg_y_coord] + i;
        }
        for(unsigned char j = 0x0 ; j < 0x8 ; ++j){ 
            if(((reg[reg_x_coord] + j)) >= 64){
                xpos = 64;
            } else{
                xpos = reg[reg_x_coord] + j;
            }
            colorPixel(xpos, ypos, sprite[i][j]);
        }
    }
    SDL_RenderPresent(this->renderer);
    return 0;
...
int Chip8::loop(){
    ...
    while(true){
        ...
        if((pc >= (unsigned int)0x200) && (pc <= (unsigned int)0x1000)){
            ...
            std::chrono::duration<double> etime = (tnow - tstart);
            ips = icount / etime.count();

            frameStart = SDL_GetPerformanceCounter();

            fprintf(stdout , "PC : %x ; Instruction : %x ; RAM[pc] : %x ; RAM[pc + 1] : %x ; IPS : %d\n" , pc , instruction , RAM[pc] , RAM[pc + 1], ips);
        ...
        this->setTimers();
        while ((SDL_GetPerformanceCounter() - frameStart) < COUNTS_PER_INSTRUCTION) {
            continue;
        }
    };
    return 0;
